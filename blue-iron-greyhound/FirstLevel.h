...
public:
	...
	{
		//Input System
		inputSystem = new SDLInputSystem();

		//physics system.
		collisionsystem = new IronRiftsPhysicsSystem();

		//Audio System
		audioSystem = new IrrKlangAudioSystem("audioSystem");

		//camera set up
		cameraComponent = new Camera("camera");
	...
	void setup()
	{
		...
		Player = new GameObject("Player 1");
		...
		particleRender = new ParticleRenderer(cameraComponent);
		particleRender->init();

		//bullet itself
		bullet = new bulletParticle(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), 200, "../../assets/tex/rainTex.png", particleRender); //(colour, numOfParticles, texture, ParticleRenderer)
	...
	//Audio System
		IrrKlangAudioSystem * audioSystem;
